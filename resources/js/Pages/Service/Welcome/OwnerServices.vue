<template>
    <service-layout>
      ...
      <div v-for="category in data.categories" 
         ...
         <Link
         :href="route('welcome.services.category',category.slug)"
         >
                {{category.title}}
         </Link>
      </div>
        ...
        <div class="px-2">
          <div id="services" v-if="data.services.data">
            <p class="text-2xl font-extrabold text-black border-b-4">
              All Available Services by {{ data.owner.name }}
            </p>
            <all-services
              v-if="data.services"
              :heading="'All Services Supplied by ' + data.owner.name"
  ...
  import FeaturedCategoryServices from "@/Components/Service/FeaturedCategoryServices.vue";
